...
import pandas as pd
import warnings
from svi import SVIIH
from svi1 import SVIIRT
...
if __name__=='__main__':
    ...
    train_idx_lst,test_idx_lst = get_splits(df['user_id'], args.n_splits)
    for i in range(args.n_splits):
        ...
        if(args.model=='SVIRT' or args.model=='SVIRTH-SC' or args.model=='SVIRTH'):
            ...
            alpha_loc_arr = pyro.get_param_store().get_param('loc_alpha').data.numpy()
            models = models.detach().numpy()
            items = items.detach().numpy()
            responses = df_train['correct'].values
            exp = (theta_loc_arr[models] - diff_loc_arr[items])*alpha_loc_arr[items]
            e_max = np.max(exp)
            if(args.model=='SVIRT'):
                probs = expit(exp)
    ...
    if(args.w):    
        ...
        op_df.to_csv('./data/{}/Prediction_metrics_2pm{}.csv'.format(args.dataset,args.model),index=False)
